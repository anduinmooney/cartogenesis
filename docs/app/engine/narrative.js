...
function makeVariedPicker(rng     ) {
  const last = new Map                ();
  return function pick(key        , bank                   )         {
...
export function generateNarrative(
  ...
  const voice = rng.pick(["plain", "wry", "grave"]         );
  ...
  const clashes = new Map                (); // "A|B" sorted → meetings
  const repulsedPairs = new Set        (); // pairs with an earlier failed invasion

  const clashKey = (a        , b        ) => (a < b ? `${a}|${b}` : `${b}|${a}`);
  ...
  const timePhrase = (year        , prevYear               )         => {
    if (prevYear === null) {
  ...
  const opening           = [cap(pick("open", OPENINGS[voice]))];
  opening.push(
    cap(
