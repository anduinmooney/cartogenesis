...
/**
 * Build an SVG poster string. `backgroundPng` is any rendered map for this
 * world (terrain or biome); it is embedded as the base layer, then labeled.
 */
export function worldPosterSVG(
  ...
  const dataUri = `data:image/png;base64,${backgroundPng.toString("base64")}`;
