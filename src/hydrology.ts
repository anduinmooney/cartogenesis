...
export interface WaterLayer {
  /** 1 where the cell is connected ocean, else 0. */
  oceanMask: Uint8Array;
  /** 1 where the cell is an inland lake (below sea level, not ocean), else 0. */
  lakeMask: Uint8Array;
  /** 1 where a land cell is 4-adjacent to ocean (the coastline), else 0. */
  coast: Uint8Array;
  /** Cell-step distance from each cell to the nearest ocean (ocean = 0). */
  distToOcean: Grid;
  /** Number of distinct lakes (connected components of lakeMask). */
  lakeCount: number;
  /** Fraction of the map that is connected ocean. */
  oceanFraction: number;
  /** Fraction of the map that is inland lake. */
  lakeFraction: number;
}

const NEIGHBORS4: ReadonlyArray<[number, number]> = [
  [1, 0],
  [-1, 0],
  [0, 1],
  [0, -1],
 ...
 */
export function analyzeWater(elevation: Grid, seaLevel: number): WaterLayer {
  ...
  const stack: number[] = [];
  const pushIfSea = (x: number, y: number) => {
    const i = y * width + x;
