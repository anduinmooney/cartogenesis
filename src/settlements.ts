...
export interface Settlement {
  ...
  y: number;
  name: string;
  /** Literal reading of the name, e.g. `"sea-haven"`. */
  gloss: string;
  regionId: number;
  tier: SettlementTier;
  isPort: boolean;
...
export function generateSettlements(
  ...
  const placed: Array<{ x: number; y: number; i: number; score: number }> = [];
  ...
  const isPort = (x: number, y: number): boolean => {
  ...
  const settlements: Settlement[] = placed.map((p, rank) => {
