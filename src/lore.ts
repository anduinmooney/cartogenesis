...
export interface LoreLayer {
  ...
  rulers: Ruler[];
  figures: Figure[];
  /** Region id → one-line description. */
  regionDescriptions: Record<number, string>;
  /** The house ruling the capital's realm, if any. */
  capitalHouse: string;
}
...
export function generateLore(
  ...
  const townsByRegion = new Map<number, Settlement[]>();
...
function makeFigures(
  ...
  type Spec = { role: string; ok: boolean; make: () => string; region: number };
  const specs: Spec[] = [
    {
      role: "the Explorer",
