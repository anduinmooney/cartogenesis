...
export interface RoadEdge {
  ...
  cost: number;
  path: number[]; // cell indices from a to b
}

export interface RoadLayer {
  roadMask: Uint8Array;
  edges: RoadEdge[];
  /** Total road length in cells. */
  length: number;
}
...
export function generateRoads(
  ...
  const passable = (i: number) => water.oceanMask[i] === 0;
  // Candidate edges between territories, keyed by "min-max" settlement pair.
  const cand = new Map<string, RoadEdge>();
...
function reconstruct(i: number, ni: number, prev: Int32Array): number[] {
  const left: number[] = [];
  let c = i;
  while (c !== -1) {
    left.push(c);
