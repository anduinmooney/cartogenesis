...
export interface Journey {
  title: string;
  /** Who walks, and why — the frame. */
  opening: string[];
  legs: JourneyLeg[];
  /** Towns a road reaches but the account only lists; and towns no road reaches. */
  closing: string[];
}
...
interface LegFacts {
  ...
  volcanoNear?: Volcano;
  craterLakeNear?: Volcano;
  ruinNear?: { name: string; fate: string; year: number };
}

export function generateJourney(input: JourneyInput, cfg: { seed: number }): Journey {
  ...
  const factsOf = (path: number[], fromId: number, toId: number): LegFacts => {
    ...
    let inRiver = false;
    let maxE = -Infinity;
    const biomeTally = new Map<number, number>();
    for (const i of path) {
    ...
    let craterLakeNear: Volcano | undefined;
  ...
  const tellLeg = (from: Settlement, to: Settlement, path: number[], backtrackFrom?: Settlement): string => {
  ...
  const visited = new Set<number>([capital.id]);
  const summarized: string[] = [];
  let position = capital;

  const walk = (at: Settlement) => {
