...
import { composeName, glossPhrase } from "./language.ts";
import { BIOME_UNDERFOOT } from "./journey.ts";
...
import type { Volcano } from "./volcanoes.ts";
...
export interface ExpeditionLeg {
  /** "land" is walked; "sea" is sailed (the route crossed open water). */
  kind: "land" | "sea";
  /** Cell indices of this leg, in walking order. */
  path: number[];
  /** The day the leg begins, 1-based. */
  day: number;
  /** A settlement reached at the end of this leg, if any. */
  waypoint?: string;
  text: string;
}
...
export interface Expedition {
  ok: boolean;
  title: string;
  /** Who was chartered, and to do what. */
  opening: string;
  legs: ExpeditionLeg[];
  closing: string;
  /** Total days on the road (and the water). */
  days: number;
  /** The whole route, concatenated, for drawing. */
  path: number[];
  fromId: number;
  toId: number;
}
 ...
 */
function findRoute(input: ExpeditionInput, start: number, goal: number): number[] {
...
interface LegFacts {
  ...
  onRoadFraction: number;
  volcanoNear?: Volcano;
  ruinNear?: { name: string; fate: string; year: number };
}
...
export function charterExpedition(
  ...
  const pick = (bank: readonly string[]) => rng.pick(bank);

  const byId = new Map(settlements.map((s) => [s.id, s]));
  const from = byId.get(fromId);
  const to = byId.get(toId);
  const fail = (title: string, opening: string): Expedition => ({
    ok: false,
    title,
    opening,
    legs: [],
  ...
  const cellTown = new Map<number, Settlement>();
  ...
  const MAXLEG = 55;
  const legsRaw: Array<{ kind: "land" | "sea"; path: number[]; waypoint?: Settlement }> = [];
  let cur: number[] = [path[0]];
  let curKind: "land" | "sea" = isWater(path[0]) ? "sea" : "land";
  const flush = (waypoint?: Settlement) => {
    if (cur.length > 1) legsRaw.push({ kind: curKind, path: cur, waypoint });
    cur = [cur[cur.length - 1]];
  };
  for (let k = 1; k < path.length; k++) {
    const i = path[k];
    const kind: "land" | "sea" = isWater(i) ? "sea" : "land";
    if (kind !== curKind) {
      flush();
      curKind = kind;
    }
  ...
  const factsOf = (cells: number[]): LegFacts => {
    const regionSeq: string[] = [];
    let fords = 0;
    let inRiver = false;
    let maxE = -Infinity;
    let onRoad = 0;
    ...
    let dominantBiome = Biome.Grassland as number;
    let best = -1;
    ...
    let volcanoNear: Volcano | undefined;
    ...
    let ruinNear: LegFacts["ruinNear"];
