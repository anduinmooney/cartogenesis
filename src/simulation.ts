...
export type RealmStatus = "ascendant" | "diminished" | "extinct";
...
export interface SimEvent {
  ...
  type:
    ...
    | "goldenage";
  text: string;
  /** Map location the event happened at (region centroid), for "fly to". */
  x: number;
  y: number;
}
...
const BIOME_CAPACITY: Record<number, number> = {
  [Biome.Ocean]: 0,
  [Biome.Lake]: 0,
  [Biome.Grassland]: 1.0,
...
interface Realm {
  ...
  name: string;
  languageId: string;
  /** 0.6 (timid) … 1.4 (warlike). Gives realms character and breaks stalemates. */
  aggression: number;
  seatRegion: number;
  regions: Set<number>;
  alive: boolean;
...
export function generateSimulation(
  ...
  const prosperity: Record<number, number> = {};
  const wealthByRegion = new Map<number, number[]>();
  ...
  const seatByRegion = new Map<number, Settlement>();
  ...
  const faith: Record<number, number> = { ...religion.regionFaith };
  const faithName = (fid: number) =>
    religion.faiths.find((f) => f.id === fid)?.name ?? "the old ways";

  const strengthOf = (realm: Realm): number => {
    let s = 0;
   ...
   */
  const aggressionCurve = (a: number): number => a * Math.sqrt(a);
  ...
  const projectedStrength = (realm: Realm, targetId: number): number => {
  ...
  const defenceOf = (realm: Realm, targetId: number): number => {
    const raw = strengthOf(realm);
  ...
  const ensureSeat = (realm: Realm): void => {
  ...
  for (let t = 0; t < turns; t++) {
    ...
    const opportunities: Array<{ a: number; b: number; region: number; ratio: number }> = [];
    for (const r of regs) {
  ...
  const summaries: RealmSummary[] = realms.map((r) => {
    const finalSize = r.alive ? r.regions.size : 0;
    const status: RealmStatus = !r.alive
      ? "extinct"
