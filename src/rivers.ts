...
export interface RiverLayer {
  /** Accumulated upstream rainfall for each cell (raw units). */
  flowAccum: Grid;
  /** 1 where a cell is a river, else 0. */
  riverMask: Uint8Array;
  /** Downstream neighbor index per cell; -1 for outlets and water cells. */
  flowTo: Int32Array;
  /** Fraction of the whole map that is river. */
  riverFraction: number;
  /** Largest single accumulated-flow value (the main river's mouth). */
  maxFlow: number;
}

export interface RiverConfig {
  /** Minimum accumulated flow for a cell to count as a river. */
  riverThreshold?: number;
  /** Filling increment guaranteeing a downhill path (small). */
  epsilon?: number;
}
