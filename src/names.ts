...
import { composeName, type NameOptions } from "./language.ts";

export interface Language {
  id: string;
  /** Human-facing label for the naming style (used in reports). */
  label: string;
  onsets: string[];
  nuclei: string[];
  codas: string[];
  /** Syllable-count weights, e.g. [2,3] means 2 or 3 syllables. */
  sylCounts: number[];
  /** Optional decorative suffixes appended to some names. */
  suffixes: string[];
  /** Probability a syllable carries a coda. */
  codaChance: number;
}
 ...
 */
export function makeName(lang: Language, rng: Rng, opts?: NameOptions): string {
