 ...
 */
export function placePattern(names: string[]): RegExp {
...
export function containsPlace(text: string, re: RegExp): boolean {
  re.lastIndex = 0;
  return re.test(text);
}
...
export function segmentPlaces(
  text: string,
  re: RegExp,
): Array<{ text: string; place: boolean }> {
  re.lastIndex = 0;
  const out: Array<{ text: string; place: boolean }> = [];
  let last = 0;
